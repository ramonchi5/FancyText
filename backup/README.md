...
- Adds configurable text shadows with custom color, mode, offset, size, fixed low-cost blur, and strength.
...
- **Size** expands the filled shadow shape around the normal shadow offset. It steps by 1% for finer control.
- **Strength** increases shadow opacity without repeated drawing loops.
- **Clip row** optionally keeps shadows inside their row. By default, shadows may overflow across rows and columns.
